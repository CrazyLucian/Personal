...
@interface QXHTeamOrderTableViewCell()
@property (nonatomic, strong) UIView *backView;
@property (nonatomic, strong) UIView *lineView;
@end
@implementation QXHTeamOrderTableViewCell
...
-(void)setupUI{
    ...
    [self.personHeadView makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.PersonNameLable makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.personHeadView.mas_centerY);
        make.left.equalTo(self.personHeadView.mas_right).offset(10);
    }];
    ...
    [self.zhuangTaiLable makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.PersonNameLable.mas_centerY);
    }];
    [self.headImageView makeConstraints:^(MASConstraintMaker *make) {
        make.width.height.equalTo(89);
        make.left.equalTo(10);
        make.top.equalTo(self.personHeadView.mas_bottom).offset(10);
    }];
...
-(UIImageView *)personHeadView{
    if (!_personHeadView) {
        _personHeadView = [[UIImageView alloc]init];
        _personHeadView.image = [UIImage imageNamed:@"色阶 661 拷贝"];
        [self.backView addSubview:_personHeadView];
    }
...
-(UILabel *)PersonNameLable{
    if (!_PersonNameLable) {
        ...
        _PersonNameLable.text = @"我的姓名是 XXXX";
...
- (void)bindGoodsModel:(QXHMyOrderModels*)model{
    ...
    [self.personHeadView sd_setImageWithURL:[NSURL URLWithString:model.avatar]];
